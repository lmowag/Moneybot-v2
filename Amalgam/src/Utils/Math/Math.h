...
inline float InterpTo(float flCurrent, float flTarget, float flDeltaTime,
                      float flInterpSpeed) {
  if (flInterpSpeed <= 0.f)
    return flTarget;

  const float flDist = flTarget - flCurrent;
  if (std::abs(flDist) < FLT_EPSILON)
    return flTarget;

  const float flDelta = flDist * Clamp(flDeltaTime * flInterpSpeed, 0.f, 1.f);
...
inline float SimpleSplineRemapVal(float flVal, float a, float b, float c,
                                  float d, bool bClamp = true) {
  if (a == b)
    return flVal >= b ? d : c;
...
inline void ClampAngles(Vec3 &v) {
  v.x = Clamp(NormalizeAngle(v.x), -89.f, 89.f);
...
inline void AngleVectors(const Vec3 &vAngles, Vec3 *pForward = nullptr,
...
inline Vec3 CalcAngle(const Vec3 &vFrom, const Vec3 &vTo, bool bClamp = true) {
  ...
  Vec3 vAngles = {atanf(vDelta.z / flHyp) * float(M_RADPI),
...
inline void ConcatTransforms(const matrix3x4 &mIn1, const matrix3x4 &mIn2,
                             matrix3x4 &mOut) {
  if (&mIn1 == &mOut) {
    matrix3x4 mIn1b;
...
inline std::vector<float> SolveQuartic(float a, float b, float c, float d,
                                       float e) {
  ...
  float m = SolveCubic(p,
                       powf(p, 2) / 4 + powf(b, 4) / (256.f / 3) - e +
                           b * d / 4 - powf(b, 2) * c / 16,
                       -powf(q, 2) / 8);
  if (m < 0.f)
    return vRoots;
