...
static inline std::deque<Frame_t> StackTrace(PCONTEXT pContext) {
  ...
  while (StackWalk64(IMAGE_FILE_MACHINE_AMD64, hProcess, hThread, &tStackFrame,
                     &tContext, nullptr, SymFunctionTableAccess64,
                     SymGetModuleBase64, nullptr)) {
    vTrace.push_back({.m_uAddress = tStackFrame.AddrPC.Offset});
    Frame_t &tFrame = vTrace.back();

    if (auto hBase = HINSTANCE(
            SymGetModuleBase64(hProcess, tStackFrame.AddrPC.Offset))) {
      tFrame.m_uBase = uintptr_t(hBase);

      char buffer[MAX_PATH];
      if (GetModuleBaseName(hProcess, hBase, buffer,
...

    {
      ...
      if (SymGetLineFromAddr64(hProcess, tStackFrame.AddrPC.Offset, &dwOffset,
                               &line)) {
...

    {
      ...
      if (SymGetSymFromAddr64(hProcess, tStackFrame.AddrPC.Offset, &dwOffset,
                              symbol))
...
static LONG APIENTRY ExceptionFilter(PEXCEPTION_POINTERS ExceptionInfo) {
  ...
  case STATUS_ACCESS_VIOLATION:
    sError = "ACCESS VIOLATION";
    break;
  case STATUS_STACK_OVERFLOW:
    sError = "STACK OVERFLOW";
    break;
  case STATUS_HEAP_CORRUPTION:
    sError = "HEAP CORRUPTION";
    break;
  case STATUS_RUNTIME_ERROR:
    sError = "RUNTIME ERROR";
    break;
  ...
  if (s_mAddresses.contains(ExceptionInfo->ExceptionRecord->ExceptionAddress) ||
      ...
      s_iExceptions && GetAsyncKeyState(VK_SHIFT) & 0x8000 &&
  ...
  if (auto vTrace = StackTrace(ExceptionInfo->ContextRecord); !vTrace.empty()) {
    for (int i = 0; i < vTrace.size(); i++) {
      ...
      if (tFrame.m_uBase)
        ssErrorStream << std::format("{}+{:#x}", tFrame.m_sModule,
                                     tFrame.m_uAddress - tFrame.m_uBase);
      else
        ssErrorStream << std::format("{:#x}", tFrame.m_uAddress);
      if (!tFrame.m_sFile.empty())
        ssErrorStream << std::format(" ({} L{})", tFrame.m_sFile,
                                     tFrame.m_uLine);
    ...
    ssErrorStream << U::Memory.GetModuleOffset(
  ...
  try {
    ...
    ssErrorStream << "Logged to Amalgam V2\\crash_log.txt. ";
  } catch (...) {
  }
  ...
  case STATUS_STACK_OVERFLOW:
  case STATUS_HEAP_CORRUPTION:
    SDK::Output("Unhandled exception", ssErrorStream.str().c_str(), {},
                OUTPUT_DEBUG, MB_OK | MB_ICONERROR);
  }
...
void CExceptionHandler::Unload() { RemoveVectoredExceptionHandler(s_pHandle); }