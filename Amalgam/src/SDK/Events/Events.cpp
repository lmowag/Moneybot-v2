...
bool CEventListener::Initialize() {
  std::vector<const char *> vEvents = {
      "client_beginconnect",  "client_connected",
      "client_disconnect",    "game_newmap",
      "teamplay_round_start", "scorestats_accumulated_update",
      "mvm_reset_stats",      "player_connect_client",
      "player_spawn",         "player_changeclass",
      "player_hurt",          "vote_cast",
      "item_pickup",          "revive_player_notify",
      "player_death"};
  ...
  for (auto szEvent : vEvents) {
    ...
    if (!I::GameEventManager->FindListener(this, szEvent)) {
      char szError[128];
      sprintf_s(szError, "Failed to add listener: %s", szEvent);
      U::Core.AppendFailText(szError);
      m_bFailed = true;
    }
  }
...
void CEventListener::Unload() { I::GameEventManager->RemoveListener(this); }
...
void CEventListener::FireGameEvent(IGameEvent *pEvent) {
  ...
  case FNV1A::Hash32Const("revive_player_notify"): {
    if (!Vars::Misc::MannVsMachine::InstantRevive.Value ||
        pEvent->GetInt("entindex") != I::EngineClient->GetLocalPlayer())
      return;
