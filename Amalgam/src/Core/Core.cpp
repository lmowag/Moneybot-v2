...
static inline std::string GetProcessName(DWORD dwProcessID) {
  ...
  if (char buffer[MAX_PATH]; GetModuleBaseName(hProcess, nullptr, buffer,
...
static inline bool CheckDXLevel() {
  ...
  if (mat_dxlevel->GetInt() < 90) {
    /*
    const char* sMessage = "You are running with graphics options that Amalgam
    V2 does not support. -dxlevel must be at least 90.";
    U::Core.AppendFailText(sMessage);
    SDK::Output("Amalgam V2", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE |
    ...
    */
    ...
    SDK::Output("Amalgam V2", sMessage, DEFAULT_COLOR,
...
void CCore::LogFailText() {
  try {
    ...
    m_ssFailStream << "Logged to Amalgam V2\\fail_log.txt. ";
  } catch (...) {
  }

  SDK::Output("Failed to load", m_ssFailStream.str().c_str(), {}, OUTPUT_DEBUG,
              MB_OK | MB_ICONERROR);
}
...
void CCore::Load() {
  ...
  while (true) {
    ...
    auto uDereference =
        uSignature
            ? *reinterpret_cast<uintptr_t *>(U::Memory.RelToAbs(uSignature))
            : 0;
    ...
    if (m_bUnload = m_bFailed = flTime >= 60.f) {
      AppendFailText(
          std::format("Failed to load in time:\n  {:#x} ({:#x})\n  {:#x}",
                      uDereference, uSignature, uintptr_t(hWindow))
              .c_str());
      return;
    }
  ...
  SDK::Output("Amalgam V2", "Loaded", DEFAULT_COLOR,
              OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
}
...
void CCore::Loop() {
  while (true) {
    bool bShouldUnload =
        U::KeyHandler.Down(VK_F11) && SDK::IsGameWindowInFocus() || m_bUnload;
    if (bShouldUnload)
      break;
...
void CCore::Unload() {
  ...
  SDK::Output("Amalgam V2", "Unloaded", DEFAULT_COLOR,
              OUTPUT_CONSOLE | OUTPUT_DEBUG);
}