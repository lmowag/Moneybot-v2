...
Enum(PointType, None = 0, Out = 1 << 0, In = 1 << 1, Out2 = 1 << 2,
     In2 = 1 << 3) Enum(Calculated, Pending, Good, Time, Bad)
...
private:
  ...
  std::vector<Point_t>
  GetSplashPoints(Vec3 vOrigin,
                  std::vector<std::pair<Vec3, int>> &vSpherePoints,
                  int iSimTime);
  void SetupSplashPoints(Vec3 &vPos,
                         std::vector<std::pair<Vec3, int>> &vSpherePoints,
                         std::vector<Vec3> &vSimplePoints);
  std::vector<Point_t> GetSplashPointsSimple(Vec3 vOrigin,
                                             std::vector<Vec3> &vSpherePoints,
                                             int iSimTime);

  void CalculateAngle(const Vec3 &vLocalPos, const Vec3 &vTargetPos,
                      int iSimTime, Solution_t &tOut, bool bAccuracy = true,
                      int iTolerance = -1);
  bool TestAngle(const Vec3 &vPoint, const Vec3 &vAngles, int iSimTime,
                 bool bSplash, bool bSecondTest = false);

  bool HandlePoint(const Vec3 &vOrigin, int iSimTime, float flPitch,
                   float flYaw, float flTime, const Vec3 &vPoint,
                   bool bSplash = false);
  ...
  bool CanHit(Target_t &tTarget, CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
              CBaseEntity *pProjectile);
  bool TestAngle(CBaseEntity *pProjectile, const Vec3 &vPoint, Vec3 &vAngles,
                 int iSimTime, bool bSplash);

  bool Aim(const Vec3 &vCurAngle, const Vec3 &vToAngle, Vec3 &vOut,
           int iMethod = Vars::Aimbot::General::AimType.Value);
  ...
  // Optimization: Trace Cache
  struct TraceCache_t {
    Vec3 vStart, vEnd;
    CGameTrace trace;
  };
  std::vector<TraceCache_t> m_vTraceCache = {};
  bool GetCachedTrace(const Vec3 &vStart, const Vec3 &vEnd, CGameTrace &trace);
  void AddCachedTrace(const Vec3 &vStart, const Vec3 &vEnd,
...
public:
  ...
  bool AutoAirblast(CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd,
                    CBaseEntity *pProjectile);
  float GetSplashRadius(CBaseEntity *pProjectile,
                        CTFWeaponBase *pWeapon = nullptr,
                        CTFPlayer *pPlayer = nullptr, float flScale = 1.f,
                        CTFWeaponBase *pAirblast = nullptr);
