...
std::vector<Target_t> CAimbotGlobal::ManageTargets(
    std::vector<Target_t> (*GetTargets)(CTFPlayer *pLocal,
                                        CTFWeaponBase *pWeapon),
    CTFPlayer *pLocal, CTFWeaponBase *pWeapon, int iMethod, int iMaxTargets) {
  ...
  for (auto &tTarget : vTargets) {
    if (tTarget.m_iTargetType == TargetEnum::Player &&
      ...
      if (flSpeed < 50.f)
        tTarget.m_nPriority += 3;
      else if (flSpeed < 150.f)
        tTarget.m_nPriority += 1;
      if (tTarget.m_flFOVTo < 2.f)
        tTarget.m_nPriority += 2;
    }
  }
...
void CAimbotGlobal::SortTargetsPre(std::vector<Target_t> &vTargets,
                                   int iMethod) {
  switch (iMethod) {
...
void CAimbotGlobal::SortTargetsPost(std::vector<Target_t> &vTargets,
                                    int iMethod) {
  switch (iMethod) {
  case Vars::Aimbot::General::TargetSelectionEnum::Hybrid:
    std::sort(vTargets.begin(), vTargets.end(),
...
bool CAimbotGlobal::PlayerBoneInFOV(CTFPlayer *pTarget, Vec3 vLocalPos,
                                    Vec3 vLocalAngles, float &flFOVTo,
                                    Vec3 &vPos, Vec3 &vAngleTo, int iHitboxes) {
  ...
  if (!Vars::Visuals::Removals::Interpolation.Value) {
    ...
    if (F::Backtrack.GetRecords(pTarget, vRecords) && !vRecords.empty()) {
      ...
      for (auto pRecord : vRecords) {
        if (F::EnginePrediction.m_flOldCurrentTime - pRecord->m_flSimTime >
            flLerp) {
...
bool CAimbotGlobal::IsHitboxValid(CBaseEntity *pEntity, int nHitbox,
                                  int iHitboxes) {
...
bool CAimbotGlobal::ShouldMultipoint(CBaseEntity *pEntity, int nHitbox,
                                     int iHitboxes) {
...
bool CAimbotGlobal::ShouldIgnore(CBaseEntity *pEntity, CTFPlayer *pLocal,
                                 CTFWeaponBase *pWeapon) {
  if (pEntity->IsDormant())
    return true;

  if (auto pGameRules = I::TFGameRules()) {
    if (pGameRules->m_bTruceActive() &&
        (FriendlyFire() || pLocal->m_iTeamNum() != pEntity->m_iTeamNum()))
      return true;
  }
  ...
  case ETFClassID::CTFPlayer: {
    ...
    if (F::PlayerUtils.IsIgnored(pPlayer->entindex()) ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Unprioritized &&
            !F::PlayerUtils.IsPrioritized(pPlayer->entindex()))
      return true;

    if (Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Friends &&
            H::Entities.IsFriend(pPlayer->entindex()) ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Party &&
            H::Entities.InParty(pPlayer->entindex()) ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Invulnerable &&
            pPlayer->IsInvulnerable() &&
            SDK::AttribHookValue(0, "crit_forces_victim_to_laugh", pWeapon) <=
                0 ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Invisible &&
            pPlayer->IsInvisible(Vars::Aimbot::General::IgnoreInvisible.Value /
                                 100.f) ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::DeadRinger &&
            pPlayer->m_bFeignDeathReady() ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Taunting &&
            pPlayer->IsTaunting() ||
        Vars::Aimbot::General::Ignore.Value &
                Vars::Aimbot::General::IgnoreEnum::Disguised &&
            pPlayer->InCond(TF_COND_DISGUISED))
      return true;
    if (Vars::Aimbot::General::Ignore.Value &
        Vars::Aimbot::General::IgnoreEnum::Vaccinator) {
      switch (G::PrimaryWeaponType) {
      case EWeaponType::HITSCAN:
        if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) &&
            SDK::AttribHookValue(0, "mod_pierce_resists_absorbs", pWeapon) != 0)
          return true;
        break;
  ...
  case ETFClassID::CObjectTeleporter: {
    ...
    if (!(Vars::Aimbot::General::Target.Value &
          Vars::Aimbot::General::TargetEnum::Sentry) &&
            pBuilding->IsSentrygun() ||
        !(Vars::Aimbot::General::Target.Value &
          Vars::Aimbot::General::TargetEnum::Dispenser) &&
            pBuilding->IsDispenser() ||
        !(Vars::Aimbot::General::Target.Value &
          Vars::Aimbot::General::TargetEnum::Teleporter) &&
            pBuilding->IsTeleporter())
      return true;
    ...
    if (pOwner) {
      ...
      if (Vars::Aimbot::General::Ignore.Value &
                  Vars::Aimbot::General::IgnoreEnum::Friends &&
              H::Entities.IsFriend(pOwner->entindex()) ||
          Vars::Aimbot::General::Ignore.Value &
                  Vars::Aimbot::General::IgnoreEnum::Party &&
              H::Entities.InParty(pOwner->entindex()))
        return true;
    }
  ...
  case ETFClassID::CTFGrenadePipebombProjectile: {
    ...
    if (pProjectile->m_iType() != TF_GL_MODE_REMOTE_DETONATE ||
        !pProjectile->m_bTouched())
      return true;
...
int CAimbotGlobal::GetPriority(int iIndex) {
  ...
  if (auto pEntity = I::ClientEntityList->GetClientEntity(iIndex)) {
    auto pBase = pEntity->As<CBaseEntity>();
    if (pBase->IsPlayer()) {
      auto pPlayer = pBase->As<CTFPlayer>();
      switch (pPlayer->m_iClass()) {
      case TF_CLASS_MEDIC:   iPriority += 3; break;
      case TF_CLASS_SNIPER:  iPriority += 2; break;
      case TF_CLASS_HEAVY:   iPriority += 1; break;
      default: break;
      }
    }
  }
...
bool CAimbotGlobal::ShouldHoldAttack(CTFWeaponBase *pWeapon) {
  ...
  case Vars::Aimbot::General::AimHoldsFireEnum::Always:
    if (!F::Aimbot.m_bRunningSecondary && !G::CanPrimaryAttack &&
        G::LastUserCmd->buttons & IN_ATTACK &&
        Vars::Aimbot::General::AimType.Value && !pWeapon->IsInReload())
      return true;
  }
  return false;
}

bool CAimbotGlobal::ComputeAimAngles(Vec3 vCurAngle, Vec3 vToAngle, Vec3 &vOut,
                                     int iMethod) {
  bool bReturn = false;
  switch (iMethod) {
  case Vars::Aimbot::General::AimTypeEnum::Plain:
  ...
  case Vars::Aimbot::General::AimTypeEnum::Smooth: {
    const float flLerp = Math::Clamp(
        Vars::Aimbot::General::AssistStrength.Value / 100.f, 0.f, 1.f);
    vOut = vCurAngle.LerpAngle(vToAngle, flLerp);
    bReturn = true;
    break;
  }
  case Vars::Aimbot::General::AimTypeEnum::Assistive: {
    ...
    vOut = vCurAngle - vMouseDelta +
           vMouseDelta.LerpAngle(
               vTargetDelta,
               Vars::Aimbot::General::AssistStrength.Value / 100.f);
    bReturn = true;
    break;
  }
  }
...
bool CAimbotGlobal::ValidBomb(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                              CBaseEntity *pBomb) {
  ...
  for (CEntitySphereQuery sphere(vOrigin, 300.f);
    ...
    if (pEntity == pLocal ||
        pEntity->IsPlayer() && (!pEntity->As<CTFPlayer>()->IsAlive() ||
                                pEntity->As<CTFPlayer>()->IsAGhost()) ||
        !FriendlyFire() && pEntity->m_iTeamNum() == pLocal->m_iTeamNum())
      continue;

    Vec3 vPos;
    pEntity->m_Collision()->CalcNearestPoint(vOrigin, &vPos);
    ...
    if (pEntity->IsPlayer() || pEntity->IsBuilding() || pEntity->IsNPC()) {
      ...
      if (!SDK::VisPosCollideable(
              pBomb, pEntity, vOrigin,
              pEntity->IsPlayer()
                  ? pEntity->m_vecOrigin() +
                        pEntity->As<CTFPlayer>()->GetViewOffset()
                  : pEntity->GetCenter(),
              MASK_SHOT))
        continue;
