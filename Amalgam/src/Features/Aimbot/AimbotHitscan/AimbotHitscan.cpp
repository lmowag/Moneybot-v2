...
static inline std::vector<Target_t> GetTargets(CTFPlayer *pLocal,
...

  {
    ...
    if (Vars::Aimbot::General::Target.Value &
      ...
      if (Vars::Aimbot::Hitscan::Modifiers.Value &
              Vars::Aimbot::Hitscan::ModifiersEnum::ExtinguishTeam &&
          !F::AimbotGlobal.FriendlyFire() &&
          SDK::AttribHookValue(0, "jarate_duration", pWeapon) > 0)
        eGroupType = EntityEnum::PlayerAll;
    }
    if (pWeapon->GetWeaponID() == TF_WEAPON_MEDIGUN)
      eGroupType = Vars::Aimbot::Healing::AutoHeal.Value
    ...
    for (auto pEntity : H::Entities.GetGroup(eGroupType)) {
      ...
      float flFOVTo;
      Vec3 vPos, vAngleTo;
      if (!F::AimbotGlobal.PlayerBoneInFOV(
              pEntity->As<CTFPlayer>(), vLocalPos, vLocalAngles, flFOVTo, vPos,
              vAngleTo, Vars::Aimbot::Hitscan::Hitboxes.Value))
        continue;
      ...
      vTargets.emplace_back(pEntity, TargetEnum::Player, vPos, vAngleTo,
                            flFOVTo, flDistTo,
                            iPriority + (G::AimTarget.m_iEntIndex == pEntity->entindex() ? 5 : 0));
    }
  ...
  if (Vars::Aimbot::General::Target.Value &
      Vars::Aimbot::General::TargetEnum::NPCs) {
  ...
  if (Vars::Aimbot::General::Target.Value &
      Vars::Aimbot::General::TargetEnum::Bombs) {
    for (auto pEntity : H::Entities.GetGroup(EntityEnum::WorldBomb)) {
      ...
      float flDistTo = vLocalPos.DistTo(vPos);
      vTargets.emplace_back(pEntity, TargetEnum::Bomb, vPos, vAngleTo, flFOVTo,
                            flDistTo);
    }
  }
...
int CAimbotHitscan::GetHitboxPriority(int nHitbox, CTFPlayer *pLocal,
                                      CTFWeaponBase *pWeapon,
                                      CBaseEntity *pTarget) {
  if (!F::AimbotGlobal.IsHitboxValid(pTarget, nHitbox,
                                     Vars::Aimbot::Hitscan::Hitboxes.Value))
    return -1;
  ...
  if (pTarget->IsPlayer()) {
    ...
    case TF_WEAPON_SNIPERRIFLE_CLASSIC: {
      ...
      if (G::CanHeadshot ||
          pLocal->InCond(TF_COND_AIMING) &&
              (pSniperRifle->GetRifleType() == RIFLE_JARATE &&
                   SDK::AttribHookValue(0, "jarate_duration", pWeapon) > 0 ||
               Vars::Aimbot::Hitscan::Modifiers.Value &
                   Vars::Aimbot::Hitscan::ModifiersEnum::WaitForHeadshot))
        bHeadshot = true;
      break;
    }
    case TF_WEAPON_REVOLVER: {
      if (SDK::AttribHookValue(0, "set_weapon_mode", pWeapon) == 1 &&
          (pWeapon->AmbassadorCanHeadshot() ||
           Vars::Aimbot::Hitscan::Modifiers.Value &
               Vars::Aimbot::Hitscan::ModifiersEnum::WaitForHeadshot))
        bHeadshot = true;
    }
    ...
    if (Vars::Aimbot::Hitscan::Hitboxes.Value &
            Vars::Aimbot::Hitscan::HitboxesEnum::BodyaimIfLethal &&
        bHeadshot) {
      ...
      case TF_WEAPON_SNIPERRIFLE_CLASSIC: {
        ...
        int iDamage =
            std::ceil(std::max(pSniperRifle->m_flChargedDamage(), 50.f) *
                      pSniperRifle->GetBodyshotMult(pPlayer));
        if (pPlayer->m_iHealth() <= iDamage)
          bHeadshot = false;
        break;
      }
      case TF_WEAPON_REVOLVER: {
        if (SDK::AttribHookValue(0, "set_weapon_mode", pWeapon) == 1) {
          ...
          int iDamage = std::ceil(
              Math::RemapVal(flDistTo, 90.f, 900.f, 60.f, 21.f) * flMult);
          if (pPlayer->m_iHealth() <= iDamage)
            bHeadshot = false;
        }
      }
  ...
  bool bHeadOnly =
      bHeadshot && Vars::Aimbot::Hitscan::Hitboxes.Value &
                       Vars::Aimbot::Hitscan::HitboxesEnum::HeadshotOnly;
  ...
  case HITBOX_PELVIS:
    return iMiscPriority;
  }

  return iLimbPriority;
};

int CAimbotHitscan::CanHit(Target_t &tTarget, CTFPlayer *pLocal,
                           CTFWeaponBase *pWeapon) {
  if (Vars::Aimbot::General::Ignore.Value &
      ...
      H::Entities.GetChoke(tTarget.m_pEntity->entindex()) >
          Vars::Aimbot::General::TickTolerance.Value)
    return false;
  ...
  if (!pHDR)
    return false;
  auto pSet =
      pHDR->pHitboxSet(tTarget.m_pEntity->As<CBaseAnimating>()->m_nHitboxSet());
  if (!pSet)
    return false;
  ...
  if (F::Backtrack.GetRecords(tTarget.m_pEntity, vRecords)) {
    ...
    F::Backtrack.m_tRecord = {tTarget.m_pEntity->m_flSimulationTime(),
                              tTarget.m_pEntity->m_vecOrigin(), Vec3(), Vec3()};
    if (!tTarget.m_pEntity->SetupBones(F::Backtrack.m_tRecord.m_aBones,
                                       MAXSTUDIOBONES, BONE_USED_BY_ANYTHING,
  ...
  if (Vars::Aimbot::Hitscan::PeekAmount.Value && pWeapon->GetWeaponSpread()) {
    ...
    case Vars::Aimbot::Hitscan::PeekCheckEnum::Off:
      break;
    case Vars::Aimbot::Hitscan::PeekCheckEnum::DoubletapOnly:
      bPeekCheck = F::Ticks.GetTicks(pWeapon);
      break;
      ...
      bPeekCheck
          ? m_vEyePos +
                pLocal->m_vecVelocity() *
                    TICKS_TO_TIME(-Vars::Aimbot::Hitscan::PeekAmount.Value)
          : Vec3();
  ...
  for (auto pRecord : vRecords) {
    ...
    if (pWeapon->GetWeaponID() == TF_WEAPON_LASER_POINTER) {
      ...
      float flBoneScale =
          std::max(Vars::Aimbot::Hitscan::BoneSizeMinimumScale.Value,
      ...
      const matrix3x4 mTransform = {{1, 0, 0, tTarget.m_vPos.x},
                                    {0, 1, 0, tTarget.m_vPos.y},
                                    {0, 0, 1, tTarget.m_vPos.z}};

      tTarget.m_vPos += tTarget.m_pEntity->GetOffset() / 2;
      if (m_vEyePos.DistToSqr(tTarget.m_vPos) > flMaxRange)
        break;

      if (SDK::VisPosWorld(pLocal, tTarget.m_pEntity, m_vEyePos,
                           tTarget.m_vPos)) {
        Vec3 vAngles;
        bool bChanged =
            Aim(G::CurrentUserCmd->viewangles,
                Math::CalcAngle(m_vEyePos, tTarget.m_vPos), vAngles);
        Vec3 vForward;
        Math::AngleVectors(vAngles, &vForward);
        float flDist = m_vEyePos.DistTo(tTarget.m_vPos);

        if (!bChanged || Math::RayToOBB(m_vEyePos, vForward, vCheckMins,
                                        vCheckMaxs, mTransform) &&
                             SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos,
                                         m_vEyePos + vForward * flDist)) {
          tTarget.m_vAngleTo = vAngles;
          tTarget.m_pRecord = pRecord;
          return true;
        } else if (iReturn == 2
                       ? vAngles.DeltaAngle(G::CurrentUserCmd->viewangles)
                                 .Length2D() <
                             tTarget.m_vAngleTo
                                 .DeltaAngle(G::CurrentUserCmd->viewangles)
                                 .Length2D()
                       : true)
          tTarget.m_vAngleTo = vAngles;
        iReturn = 2;
    ...
    if (tTarget.m_iTargetType == TargetEnum::Player) {
      ...
      for (int nHitbox = 0; nHitbox < pSet->numhitboxes; nHitbox++) {
        int iPriority =
            GetHitboxPriority(nHitbox, pLocal, pWeapon, tTarget.m_pEntity);
        if (iPriority == -1)
          continue;
      ...
      std::sort(vHitboxes.begin(), vHitboxes.end(),
                [&](const auto &a, const auto &b) -> bool {
                  return std::get<2>(a) < std::get<2>(b);
                });

      float flModelScale =
          tTarget.m_pEntity->As<CBaseAnimating>()->m_flModelScale();
      ...
      Vec3 vHullMins =
          (pViewVectors ? pViewVectors->m_vHullMin : Vec3(-24, -24, 0)) *
          flModelScale;
      Vec3 vHullMaxs =
          (pViewVectors ? pViewVectors->m_vHullMax : Vec3(24, 24, 82)) *
          flModelScale;

      const matrix3x4 mTransform = {{1, 0, 0, pRecord->m_vOrigin.x},
                                    {0, 1, 0, pRecord->m_vOrigin.y},
                                    {0, 0, 1, pRecord->m_vOrigin.z}};

      const bool bTriggerbot =
          (Vars::Aimbot::General::AimType.Value ==
               Vars::Aimbot::General::AimTypeEnum::Smooth ||
           Vars::Aimbot::General::AimType.Value ==
               Vars::Aimbot::General::AimTypeEnum::Assistive) &&
          !Vars::Aimbot::General::AssistStrength.Value;
      const float flMultipointScale =
          Vars::Aimbot::Hitscan::MultipointScale.Value / 100.f;
      ...
      for (auto &[pBox, nHitbox, _] : vHitboxes) {
        ...
        Vec3 vAngle;
        Math::MatrixAngles(aBones[pBox->bone], vAngle);

        Vec3 vOffset;
        {
          Vec3 vOrigin, vCenter;
          Math::VectorTransform({}, aBones[pBox->bone], vOrigin);
          Math::VectorTransform((vMins + vMaxs) / 2, aBones[pBox->bone],
                                vCenter);
          vOffset = vCenter - vOrigin;
        }

        std::vector<Vec3> vPoints = {Vec3()};
        if (F::AimbotGlobal.ShouldMultipoint(
                tTarget.m_pEntity, nHitbox,
                Vars::Aimbot::Hitscan::MultipointHitboxes.Value)) {
          flBoneScale =
              std::max(flBoneScale,
                       Vars::Aimbot::Hitscan::MultipointScale.Value / 100.f);
          if (!bTriggerbot) {
        ...
        for (auto &vPoint : vPoints) {
          ...
          Vec3 vAngles;
          bool bChanged = Aim(G::CurrentUserCmd->viewangles,
                              Math::CalcAngle(m_vEyePos, vOrigin), vAngles);
          Vec3 vForward;
          Math::AngleVectors(vAngles, &vForward);
          float flDist = m_vEyePos.DistTo(vOrigin);

          if (bChanged ||
              SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos, vOrigin)) {
            // for the time being, no vischecks against other hitboxes
            if ((!bChanged ||
                 Math::RayToOBB(m_vEyePos, vForward, vCheckMins, vCheckMaxs,
                                aBones[pBox->bone], flModelScale) &&
                     SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos,
                                 m_vEyePos + vForward * flDist)) &&
                Math::RayToOBB(m_vEyePos, vForward, vHullMins, vHullMaxs,
                               mTransform)) {
              ...
              return true;
            } else if (bChanged && SDK::VisPos(pLocal, tTarget.m_pEntity,
                                               m_vEyePos, vOrigin)) {
              if (iReturn != 2 ||
                  vAngles.DeltaAngle(G::CurrentUserCmd->viewangles).Length2D() <
                      tTarget.m_vAngleTo
                          .DeltaAngle(G::CurrentUserCmd->viewangles)
                          .Length2D())
                tTarget.m_vAngleTo = vAngles;
      ...
      for (auto &vPoint : vPoints) {
        ...
        if (bChanged ||
          ...
          if (!bChanged || Math::RayToOBB(m_vEyePos, vForward, vCheckMins,
            ...
            if (iReturn != 2 ||
                vAngles.DeltaAngle(G::CurrentUserCmd->viewangles).Length2D() <
                    tTarget.m_vAngleTo.DeltaAngle(G::CurrentUserCmd->viewangles)
                        .Length2D())
              tTarget.m_vAngleTo = vAngles;
            iReturn = 2;
          }
...
bool CAimbotHitscan::ShouldFire(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                                CUserCmd *pCmd, const Target_t &tTarget) {
  ...
  if (Vars::Aimbot::Hitscan::Modifiers.Value &
    ...
    case TF_WEAPON_SNIPERRIFLE_DECAP:
      if (!G::CanHeadshot && pLocal->InCond(TF_COND_AIMING) &&
          pWeapon->As<CTFSniperRifle>()->GetRifleType() != RIFLE_JARATE)
        return false;
      break;
    case TF_WEAPON_SNIPERRIFLE_CLASSIC:
      if (!G::CanHeadshot)
  ...
  if (Vars::Aimbot::Hitscan::Modifiers.Value &
      Vars::Aimbot::Hitscan::ModifiersEnum::WaitForCharge) {
    switch (pWeapon->GetWeaponID()) {
    case TF_WEAPON_SNIPERRIFLE:
    case TF_WEAPON_SNIPERRIFLE_DECAP:
    case TF_WEAPON_SNIPERRIFLE_CLASSIC: {
      ...
      if (!pLocal->InCond(TF_COND_AIMING) ||
          pSniperRifle->m_flChargedDamage() == 150.f)
        break;
      ...
      if (tTarget.m_pEntity->IsPlayer()) {
        ...
        if (tTarget.m_nAimedHitbox == HITBOX_HEAD &&
          ...
          int iDamage =
              std::ceil(std::max(pSniperRifle->m_flChargedDamage(), 50.f) *
                        pSniperRifle->GetHeadshotMult(pPlayer));
          if (pPlayer->m_iHealth() <= iDamage &&
              (G::CanHeadshot || pLocal->IsCritBoosted()))
            break;
        } else {
          int iDamage =
              std::ceil(std::max(pSniperRifle->m_flChargedDamage(), 50.f) *
...
bool CAimbotHitscan::Aim(Vec3 vCurAngle, Vec3 vToAngle, Vec3 &vOut,
                         int iMethod) {
  ...
  bool bReturn = false;
  vToAngle -= vPunch;
  return F::AimbotGlobal.ComputeAimAngles(vCurAngle, vToAngle, vOut, iMethod);
}
...
static inline void DrawVisuals(CTFPlayer *pLocal, Target_t &tTarget,
                               int nWeaponID) {
  if (G::Attacking == 1 && nWeaponID != TF_WEAPON_LASER_POINTER) {
    bool bLine = Vars::Visuals::Line::Enabled.Value;
    bool bBoxes = Vars::Visuals::Hitbox::BonesEnabled.Value &
                  Vars::Visuals::Hitbox::BonesEnabledEnum::OnShot;
    if (G::CanPrimaryAttack && (bLine || bBoxes)) {
      ...
      if (bLine) {
        ...
        Vec3 vForward;
        Math::AngleVectors(tTarget.m_vAngleTo + pLocal->m_vecPunchAngle(),
                           &vForward);
      ...
      if (bBoxes) {
        auto vBoxes =
            F::Visuals.GetHitboxes(tTarget.m_pRecord->m_aBones,
                                   tTarget.m_pEntity->As<CBaseAnimating>(), {},
...
void CAimbotHitscan::Run(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                         CUserCmd *pCmd) {
  ...
  if (!Vars::Aimbot::General::AimType.Value ||
      !F::AimbotGlobal.ShouldAim() &&
          (nWeaponID != TF_WEAPON_MINIGUN ||
           pWeapon->As<CTFMinigun>()->m_iWeaponState() == AC_STATE_FIRING ||
           pWeapon->As<CTFMinigun>()->m_iWeaponState() == AC_STATE_SPINNING))
    return;

  switch (nWeaponID) {
  case TF_WEAPON_MINIGUN:
    if (Vars::Aimbot::Hitscan::Modifiers.Value &
        Vars::Aimbot::Hitscan::ModifiersEnum::AutoRev)
      pCmd->buttons |= IN_ATTACK2;
    if (pWeapon->As<CTFMinigun>()->m_iWeaponState() != AC_STATE_FIRING &&
        pWeapon->As<CTFMinigun>()->m_iWeaponState() != AC_STATE_SPINNING)
      return;
    break;
  }
  ...
  case TF_WEAPON_SNIPERRIFLE_DECAP: {
    ...
    if (Vars::Aimbot::Hitscan::Modifiers.Value &
            Vars::Aimbot::Hitscan::ModifiersEnum::AutoScope &&
        !bScoped) {
      pCmd->buttons |= IN_ATTACK2;
      return;
    } else if (Vars::Aimbot::Hitscan::Modifiers.Value &
                   Vars::Aimbot::Hitscan::ModifiersEnum::ScopedOnly &&
               !bScoped)
      return;
    else if (!bScoped &&
             SDK::AttribHookValue(0, "sniper_only_fire_zoomed", pWeapon))
      return;
    break;
  }
  case TF_WEAPON_SNIPERRIFLE_CLASSIC:
  ...
  for (auto &tTarget : vTargets) {
    ...
    if (ShouldFire(pLocal, pWeapon, pCmd, tTarget) || F::Ticks.m_bDoubletap) {
      switch (nWeaponID) {
      case TF_WEAPON_MEDIGUN:
        ...
        break;
      case TF_WEAPON_SNIPERRIFLE_CLASSIC:
        if (pWeapon->As<CTFSniperRifle>()->m_flChargedDamage() &&
            pLocal->m_hGroundEntity())
          pCmd->buttons &= ~IN_ATTACK;
        break;
      ...
      if (Vars::Aimbot::Hitscan::Modifiers.Value &
          ...
          pWeapon->GetWeaponSpread() != 0.f &&
          !pLocal->InCond(TF_COND_RUNE_PRECISION) &&
          m_vEyePos.DistTo(tTarget.m_vPos) >
              Vars::Aimbot::Hitscan::TapfireDistance.Value) {
        const float flTimeSinceLastShot =
            (pLocal->m_nTickBase() * TICK_INTERVAL) -
            pWeapon->m_flLastFireTime();
        if (flTimeSinceLastShot <=
            (pWeapon->GetBulletsPerShot() > 1 ? 0.25f : 1.25f))
          pCmd->buttons &= ~IN_ATTACK;
      }
    }
    ...
    if (G::Attacking == 1 && nWeaponID != TF_WEAPON_LASER_POINTER) {
      if (tTarget.m_pEntity->IsPlayer())
        F::Resolver.HitscanRan(pLocal, tTarget.m_pEntity->As<CTFPlayer>(),
                               pWeapon, tTarget.m_nAimedHitbox);
    ...
    if (G::SilentAngles) {
      switch (nWeaponID) {
      case TF_WEAPON_MEDIGUN:
        // case TF_WEAPON_LASER_POINTER: // we can psilent with the wrangler
        // though probably with some hacks
