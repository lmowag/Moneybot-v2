...
template <class T>
void CConfigs::SaveJson(boost::property_tree::ptree &t, const std::string &s,
                        const T &v) {
  t.put(s, v);
}
...
template <>
void CConfigs::SaveJson(boost::property_tree::ptree &t, const std::string &s,
                        const std::vector<std::pair<std::string, Color_t>> &v) {
  boost::property_tree::ptree tChild;
  for (auto &[m, c] : v) {
...
template <class T>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, IntRange_t &v) {
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, FloatRange_t &v) {
  if (auto tChild = t.get_child_optional(s)) {
    LoadJson(*tChild, "Min", v.Min);
    LoadJson(*tChild, "Max", v.Max);
  }
...
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s,
                        std::vector<std::pair<std::string, Color_t>> &v) {
  if (auto tChild = t.get_child_optional(s)) {
    v.clear();
  ...
  for (auto it = v.begin(); it != v.end();) {
    ...
    bool bValid = uHash != FNV1A::Hash32Const("None") &&
                  (uHash == FNV1A::Hash32Const("Original") ||
                   F::Materials.m_mMaterials.contains(uHash));
    if (bValid) {
      int iCount = 0;
      for (auto it2 = v.begin(); it2 != it; it2++) {
        if (FNV1A::Hash32(it2->first.c_str()) == uHash)
          iCount++;
      }
      bValid = iCount == 0;
    }
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, Gradient_t &v) {
  if (auto tChild = t.get_child_optional(s)) {
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, DragBox_t &v) {
  if (auto tChild = t.get_child_optional(s)) {
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, WindowBox_t &v) {
  if (auto tChild = t.get_child_optional(s)) {
    LoadJson(*tChild, "x", v.x);
    LoadJson(*tChild, "y", v.y);
...
template <class T>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, ConfigVar<T> *c, int i) {
  LoadJson(t, s, c->Map[i]);
}

template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, ConfigVar<int> *c, int i) {
  auto &v = c->Map[i];
  LoadJson(t, s, v);
  ...
  if (!c->m_vValues.empty()) {
    ...
    if (!(c->m_iFlags & SLIDER_PRECISION))
      v = float(v) - fnmodf(float(v) - c->m_unStep.i / 2.f, c->m_unStep.i) +
          c->m_unStep.i / 2.f;
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, ConfigVar<float> *c, int i) {
  auto &v = c->Map[i];
  LoadJson(t, s, v);
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, ConfigVar<IntRange_t> *c, int i) {
  auto &v = c->Map[i];
  LoadJson(t, s, v);

  if (!(c->m_iFlags & SLIDER_PRECISION)) {
    v.Min = float(v.Min) -
            fnmodf(float(v.Min) - c->m_unStep.i / 2.f, c->m_unStep.i) +
            c->m_unStep.i / 2.f;
    v.Max = float(v.Max) -
            fnmodf(float(v.Max) - c->m_unStep.i / 2.f, c->m_unStep.i) +
            c->m_unStep.i / 2.f;
  }
  if (c->m_iFlags & SLIDER_CLAMP) {
...
template <>
void CConfigs::LoadJson(const boost::property_tree::ptree &t,
                        const std::string &s, ConfigVar<FloatRange_t> *c,
                        int i) {
  auto &v = c->Map[i];
  LoadJson(t, s, v);

  if (!(c->m_iFlags & SLIDER_PRECISION)) {
    v.Min = v.Min - fnmodf(v.Min - c->m_unStep.f / 2, c->m_unStep.f) +
            c->m_unStep.f / 2;
    v.Max = v.Max - fnmodf(v.Max - c->m_unStep.f / 2, c->m_unStep.f) +
            c->m_unStep.f / 2;
  }
  if (c->m_iFlags & SLIDER_CLAMP) {
...
CConfigs::CConfigs() {
  m_sConfigPath = std::filesystem::current_path().string() + "\\Amalgam V2\\";
...
template <class T>
static inline void SaveMain(BaseVar *&pBase,
...
#define Save(t)                                                                \
  if (IsType(t))                                                               \
    SaveMain<t>(pBase, tSub);

template <class T>
static inline void LoadMain(BaseVar *&pBase,
                            boost::property_tree::ptree &tTree) {
  auto pVar = pBase->As<T>();
  ...
  if (auto tMap = tTree.get_child_optional(pVar->Name())) {
    for (auto &[sKey, _] : *tMap) {
      ...
      if (iBind == DEFAULT_BIND ||
          F::Binds.m_vBinds.size() > iBind && !(pVar->m_iFlags & NOBIND)) {
  ...
  try {
...

    {
      ...
      for (auto &pBase : G::Vars) {
        ...
        Save(bool) else Save(int) else Save(float) else Save(IntRange_t) else Save(
            FloatRange_t) else Save(std::string) else Save(MaterialLayers_t) else Save(
            Color_t) else Save(Gradient_t) else Save(DragBox_t) else Save(
            WindowBox_t)
      }
...

    {
      ...
      for (int iID = 0; iID < F::Groups.m_vGroups.size(); iID++) {
        ...
        SaveJson(tChild, "OffscreenArrowsOffset",
                 tGroup.m_iOffscreenArrowsOffset);
        SaveJson(tChild, "OffscreenArrowsMaxDistance",
                 tGroup.m_flOffscreenArrowsMaxDistance);
    ...
    m_sCurrentConfig = sConfigName;
    m_sCurrentVisuals = "";
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Config {} saved", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Save config failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    return false;
  }
...
bool CConfigs::LoadConfig(const std::string &sConfigName, bool bNotify) {
  try {
    if (!std::filesystem::exists(m_sConfigPath + sConfigName +
                                 m_sConfigExtension)) {
    ...
    if (auto tSub = tRead.get_child_optional("Binds")) {
      ...
      SDK::Output("Amalgam V2", "Config binds not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);

    if (auto oSub = tRead.get_child_optional("Vars");
        oSub || (oSub = tRead.get_child_optional("ConVars"))) {
      auto &tSub = *oSub;
      bool bNoSave = GetAsyncKeyState(VK_SHIFT) & 0x8000;
      for (auto &pBase : G::Vars) {
        if (!bNoSave && pBase->m_iFlags & NOSAVE)
        ...
        Load(bool) else Load(int) else Load(float) else Load(IntRange_t) else Load(
            FloatRange_t) else Load(std::string) else Load(MaterialLayers_t) else Load(
            Color_t) else Load(Gradient_t) else Load(DragBox_t) else Load(
            WindowBox_t)
      }
    } else
      SDK::Output("Amalgam V2", "Config vars not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (auto tSub = tRead.get_child_optional("Groups")) {
      for (auto &[_, tChild] : *tSub) {
        ...
        LoadJson(tChild, "OffscreenArrowsOffset",
                 tGroup.m_iOffscreenArrowsOffset);
        LoadJson(tChild, "OffscreenArrowsMaxDistance",
                 tGroup.m_flOffscreenArrowsMaxDistance);
      ...
      SDK::Output("Amalgam V2", "Config groups not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Config {} loaded", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Load config failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    return false;
  }
...
template <class T>
static inline void SaveMiscMain(BaseVar *&pBase,
                                boost::property_tree::ptree &tTree) {
  F::Configs.SaveJson(tTree, pBase->Name(), pBase->As<T>()->Map[DEFAULT_BIND]);
}
#define SaveMisc(t, j)                                                         \
  if (IsType(t))                                                               \
    SaveMiscMain<t>(pBase, j);

template <class T>
static inline void LoadMiscMain(BaseVar *&pBase,
                                boost::property_tree::ptree &tTree) {
  F::Configs.LoadJson(tTree, pBase->Name(), pBase->As<T>(), DEFAULT_BIND);
}
#define LoadMisc(t, j)                                                         \
  if (IsType(t))                                                               \
    LoadMiscMain<t>(pBase, j);
  ...
  try {
...

    {
      ...
      for (auto &pBase : G::Vars) {
        ...
        SaveMisc(bool, tSub) else SaveMisc(int, tSub) else SaveMisc(float, tSub) else SaveMisc(IntRange_t, tSub) else SaveMisc(
            FloatRange_t,
            tSub) else SaveMisc(std::string,
                                tSub) else SaveMisc(MaterialLayers_t,
                                                    tSub) else SaveMisc(Color_t,
                                                                        tSub) else SaveMisc(Gradient_t,
                                                                                            tSub) else SaveMisc(DragBox_t,
                                                                                                                tSub) else SaveMisc(WindowBox_t,
                                                                                                                                    tSub)
      }
      tWrite.put_child("Vars", tSub);
    }
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Visual config {} saved", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Save visuals failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    return false;
  }
  return true;
...
bool CConfigs::LoadVisual(const std::string &sConfigName, bool bNotify) {
  try {
    if (!std::filesystem::exists(m_sVisualsPath + sConfigName +
                                 m_sConfigExtension))
      return false;
    ...
    if (auto tSub = tRead.get_child_optional("Vars"); tSub || (tSub = tRead)) {
      bool bNoSave = GetAsyncKeyState(VK_SHIFT) & 0x8000;
      for (auto &pBase : G::Vars) {
        if (!(pBase->m_iFlags & VISUAL) || !bNoSave && pBase->m_iFlags & NOSAVE)
          continue;

        LoadMisc(bool, *tSub) else LoadMisc(int, *tSub) else LoadMisc(float, *tSub) else LoadMisc(IntRange_t, *tSub) else LoadMisc(
            FloatRange_t,
            *tSub) else LoadMisc(std::string,
                                 *tSub) else LoadMisc(VA_LIST(std::
                                                                  vector<std::pair<
                                                                      std::
                                                                          string,
                                                                      Color_t>>),
                                                      *tSub) else LoadMisc(Color_t,
                                                                           *tSub) else LoadMisc(Gradient_t,
                                                                                                *tSub) else LoadMisc(DragBox_t,
                                                                                                                     *tSub) else LoadMisc(WindowBox_t,
                                                                                                                                          *tSub)
      }
    } else
      SDK::Output("Amalgam V2", "Config vars not found", ALTERNATE_COLOR,
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Visual config {} loaded", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Load visuals failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    return false;
  }
  return true;
...
template <class T> static inline void ResetMain(BaseVar *&pBase) {
  auto pVar = pBase->As<T>();

  pVar->Map = {{DEFAULT_BIND, pVar->Default}};
}
#define Reset(...)                                                             \
  if (IsType(__VA_ARGS__))                                                     \
    ResetMain<__VA_ARGS__>(pBase);
...
void CConfigs::DeleteConfig(const std::string &sConfigName, bool bNotify) {
  try {
    ...
    if (FNV1A::Hash32(m_sCurrentConfig.c_str()) ==
        FNV1A::Hash32(sConfigName.c_str()))
      LoadConfig("default", false);

    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Config {} deleted", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Remove config failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}
...
void CConfigs::ResetConfig(const std::string &sConfigName, bool bNotify) {
  try {
    ...
    for (auto &pBase : G::Vars) {
      ...
      Reset(bool) else Reset(int) else Reset(float) else Reset(IntRange_t) else Reset(FloatRange_t) else Reset(
          std::string) else Reset(std::vector<std::string>) else Reset(
          std::vector<std::pair<std::string, Color_t>>) else Reset(
          Color_t) else Reset(Gradient_t) else Reset(DragBox_t) else Reset(
          WindowBox_t)
    }
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Config {} reset", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Reset config failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}
...
void CConfigs::DeleteVisual(const std::string &sConfigName, bool bNotify) {
  try {
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Visual config {} deleted", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Remove visuals failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}
...
void CConfigs::ResetVisual(const std::string &sConfigName, bool bNotify) {
  try {
    ...
    if (bNotify)
      SDK::Output("Amalgam V2",
                  std::format("Visual config {} reset", sConfigName).c_str(),
                  DEFAULT_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Reset visuals failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}
