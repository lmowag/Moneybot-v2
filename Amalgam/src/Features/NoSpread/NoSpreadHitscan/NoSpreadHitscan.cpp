...
MAKE_SIGNATURE(CTFWeaponBaseGun_GetWeaponSpread, "client.dll",
               "48 89 5C 24 ? 57 48 83 EC ? 4C 63 91", 0x0);
...
bool CNoSpreadHitscan::ShouldRun(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                                 bool bCreateMove) {
  if (G::PrimaryWeaponType != EWeaponType::HITSCAN ||
      (bCreateMove
           ? pWeapon->GetWeaponSpread()
           : S::CTFWeaponBaseGun_GetWeaponSpread.Call<float>(pWeapon)) <= 0.f)
    return false;
...
int CNoSpreadHitscan::GetSeed(CUserCmd *pCmd) {
  static auto sv_usercmd_custom_random_seed =
      H::ConVars.FindVar("sv_usercmd_custom_random_seed");
...
float CNoSpreadHitscan::CalcMantissaStep(float flV) {
  ...
  float flNextValue =
      std::nextafter(flV, std::numeric_limits<float>::infinity());
...
void CNoSpreadHitscan::AskForPlayerPerf() {
  ...
  static Timer tTimer = {};
  if (!m_bWaitingForPlayerPerf
          ? tTimer.Run(Vars::Aimbot::General::NoSpreadInterval.Value)
          : tTimer.Run(Vars::Aimbot::General::NoSpreadBackupInterval.Value)) {
...
bool CNoSpreadHitscan::ParsePlayerPerf(const std::string &sMsg) {
  ...
  std::regex_match(
      sMsg, tMatch,
      std::regex(R"((\d+.\d+)\s\d+\s\d+\s\d+.\d+\s\d+.\d+\svel\s\d+.\d+)"));
  ...
  if (tMatch.size() == 2) {
    ...
    if (bLoopback)
      ...
      m_vTimeDeltas.push_back(m_flServerTime - m_dRequestTime +
                              TICKS_TO_TIME(1));
      while (!m_vTimeDeltas.empty() &&
             m_vTimeDeltas.size() >
                 Vars::Aimbot::General::NoSpreadAverage.Value)
        m_vTimeDeltas.pop_front();
      m_dTimeDelta = std::reduce(m_vTimeDeltas.begin(), m_vTimeDeltas.end()) /
                     m_vTimeDeltas.size();
    }
    ...
    if (flMantissaStep > m_flMantissaStep && (m_bSynced || !m_flMantissaStep)) {
      SDK::Output("Seed Prediction",
                  m_bSynced ? std::format("Synced ({})", m_dTimeDelta).c_str()
                            : "Not synced, step too low",
                  Vars::Menu::Theme::Accent.Value);
      SDK::Output("Seed Prediction",
                  std::format("Age {}; Step {}", GetFormat(m_flServerTime),
                              flMantissaStep)
                      .c_str(),
                  Vars::Menu::Theme::Accent.Value);
    }
...
void CNoSpreadHitscan::Run(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                           CUserCmd *pCmd) {
  ...
  SDK::Output(
      "CNoSpreadHitscan::Run",
      std::format("{}: {}", SDK::PlatFloatTime() + m_dTimeDelta, m_iSeed)
          .c_str(),
      {255, 0, 0});
#endif
  ...
  // credits to Amalgam V2 for average spread stuff
  float flSpread = pWeapon->GetWeaponSpread();
  int iBulletsPerShot = pWeapon->GetBulletsPerShot();
  float flFireRate =
      std::ceilf(pWeapon->GetFireRate() / TICK_INTERVAL) * TICK_INTERVAL;
  ...
  for (int iBullet = 0; iBullet < iBulletsPerShot; iBullet++) {
    ...
    {
      // if we are doubletapping and firerate is fast enough, prioritize later
      // bullets
      int iTicks = F::Ticks.GetTicks();
      if (!iTicks || iTicks < TIME_TO_TICKS(flFireRate) * 2) {
        float flTimeSinceLastShot =
            I::GlobalVars->curtime - pWeapon->m_flLastFireTime();
    ...
    const float x =
        SDK::RandomFloat(-0.5f, 0.5f) + SDK::RandomFloat(-0.5f, 0.5f);
    const float y =
        SDK::RandomFloat(-0.5f, 0.5f) + SDK::RandomFloat(-0.5f, 0.5f);
    ...
    Vec3 vFixedSpread =
        vForward + (vRight * x * flSpread) + (vUp * y * flSpread);
  ...
  const auto cFixedSpread = std::ranges::min_element(
      vBulletCorrections, [&](const Vec3 &lhs, const Vec3 &rhs) {
...
void CNoSpreadHitscan::Draw(CTFPlayer *pLocal) {
  if (!(Vars::Menu::Indicators.Value &
        Vars::Menu::IndicatorsEnum::SeedPrediction) ||
      !Vars::Aimbot::General::NoSpread.Value || !pLocal->IsAlive())
    return;
  ...
  const auto &cColor = m_bSynced ? Vars::Menu::Theme::Active.Value
                                 : Vars::Menu::Theme::Inactive.Value;

  H::Draw.StringOutlined(
      fFont, x, y, cColor, Vars::Menu::Theme::Background.Value, align,
      std::format("Uptime {}", GetFormat(m_flServerTime)).c_str());
  H::Draw.StringOutlined(
      fFont, x, y += nTall, cColor, Vars::Menu::Theme::Background.Value, align,
      std::format("Mantissa step {}", m_flMantissaStep).c_str());
  if (Vars::Debug::Info.Value)
    H::Draw.StringOutlined(fFont, x, y += nTall, cColor,
                           Vars::Menu::Theme::Background.Value, align,
                           std::format("Delta {:.3f}", m_dTimeDelta).c_str());
}