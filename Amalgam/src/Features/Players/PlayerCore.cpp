...
void CPlayerlistCore::SavePlayerlist() {
  if (!F::PlayerUtils.m_bSave ||
      F::PlayerUtils.m_bLoad) // terrible if we end up saving while loading
    return;
  ...
  try {
    ...
    SDK::Output("Amalgam V2", "Saved playerlist", DEFAULT_COLOR,
                OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Save playerlist failed", ALTERNATE_COLOR,
...
void CPlayerlistCore::LoadPlayerlist() {
  ...
  try {
    ...
    if (auto tSub = tRead.get_child_optional("Config")) {
      ...
      SDK::Output("Amalgam V2", "Playerlist config not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (auto tSub = tRead.get_child_optional("Tags")) {
      ...
      SDK::Output("Amalgam V2", "Playerlist tags not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (auto tSub = tRead.get_child_optional("Aliases")) {
      ...
      SDK::Output("Amalgam V2", "Playerlist aliases not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);

    F::PlayerUtils.m_bLoad = false;
    SDK::Output("Amalgam V2", "Loaded playerlist", DEFAULT_COLOR,
                OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Amalgam V2", "Load playerlist failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}