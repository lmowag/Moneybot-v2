...
void CMovementSimulation::Store(MoveStorage &tMoveStorage) {
  ...
  CPredictionCopy copy = {PC_NETWORKED_ONLY, tMoveStorage.m_pData,
                          PC_DATA_PACKED, tMoveStorage.m_pPlayer,
                          PC_DATA_NORMAL};
  copy.TransferData("MovementSimulationStore",
                    tMoveStorage.m_pPlayer->entindex(), pMap);
}
...
void CMovementSimulation::Reset(MoveStorage &tMoveStorage) {
  if (tMoveStorage.m_pData) {
    ...
    CPredictionCopy copy = {PC_NETWORKED_ONLY, tMoveStorage.m_pPlayer,
                            PC_DATA_NORMAL, tMoveStorage.m_pData,
                            PC_DATA_PACKED};
    copy.TransferData("MovementSimulationReset",
                      tMoveStorage.m_pPlayer->entindex(), pMap);
...
static inline void HandleMovement(CTFPlayer *pPlayer, MoveData *pLastRecord,
                                  MoveData &tCurRecord,
                                  std::deque<MoveData> &vRecords) {
  ...
  if (pLastRecord) {
    ...
    { // does this eat up fps? i can't tell currently
      ...
      SDK::TraceHull(pLastRecord->m_vOrigin,
                     pLastRecord->m_vOrigin +
                         pLastRecord->m_vVelocity * TICK_INTERVAL,
  ...
  if (pPlayer->InCond(TF_COND_SHIELD_CHARGE)) {
    ...
    SDK::FixMovement(
        &s_tDummyCmd,
        bLocal ? F::EnginePrediction.m_vAngles : pPlayer->GetEyeAngles(), {});
  ...
  case MoveEnum::Ground: {
    if (bLocal && Vars::Misc::Movement::Bunnyhop.Value &&
        G::OriginalCmd.buttons & IN_JUMP) {
      float flMaxSpeed = SDK::MaxSpeed(pPlayer, true);
      tCurRecord.m_vDirection =
          tCurRecord.m_vVelocity.Normalized2D() * flMaxSpeed;
    }
    break;
  }
  case MoveEnum::Air: {
...
void CMovementSimulation::Store() {
  for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll)) {
    ...
    Vec3 vVelocity =
        bLocal ? F::EnginePrediction.m_vVelocity : pPlayer->m_vecVelocity();
    Vec3 vOrigin =
        bLocal ? F::EnginePrediction.m_vOrigin : pPlayer->m_vecOrigin();
    Vec3 vDirection =
        bLocal ? Math::RotatePoint(F::EnginePrediction.m_vDirection, {},
                                   {0, F::EnginePrediction.m_vAngles.y, 0})
               : vVelocity.To2D();

    MoveData *pLastRecord = !vRecords.empty() ? &vRecords.front() : nullptr;
    vRecords.emplace_front(vDirection, pPlayer->m_flSimulationTime(),
                           pPlayer->IsSwimming()   ? MoveEnum::Swim
                           : pPlayer->IsOnGround() ? MoveEnum::Ground
                                                   : MoveEnum::Air,
                           vVelocity, vOrigin);
...
bool CMovementSimulation::Initialize(CBaseEntity *pEntity,
                                     MoveStorage &tMoveStorage, bool bHitchance,
                                     bool bStrafe) {
  if (!pEntity || !pEntity->IsPlayer() ||
      !pEntity->As<CTFPlayer>()->IsAlive()) {
  ...
  if (pPlayer->m_bDucked() = pPlayer->IsDucking()) {
    pPlayer->m_fFlags() &=
        ~FL_DUCKING; // breaks origin's z if FL_DUCKING is not removed
  ...
  if (pPlayer->entindex() != I::EngineClient->GetLocalPlayer()) {
    pPlayer->m_vecBaseVelocity() =
        Vec3(); // residual basevelocity causes issues
    if (pPlayer->IsOnGround())
      pPlayer->m_vecVelocity().z =
          std::min(pPlayer->m_vecVelocity().z, 0.f); // step fix
    else
      pPlayer->m_hGroundEntity() =
          nullptr; // fix for velocity.z being set to 0 even if in air
  } else if (Vars::Misc::Movement::Bunnyhop.Value &&
             G::OriginalCmd.buttons & IN_JUMP)
  ...
  bool bCalculated =
      bStrafe ? StrafePrediction(tMoveStorage, iStrafeSamples) : false;
  ...
  if (bHitchance && bCalculated && !pPlayer->m_vecVelocity().IsZero() &&
    ...
    for (size_t i = 0; i < iSamples; i++) {
      ...
      const float flYaw1 = Math::VectorAngles(pRecord1.m_vDirection).y,
                  flYaw2 = Math::VectorAngles(pRecord2.m_vDirection).y;
      const float flTime1 = pRecord1.m_flSimTime,
                  flTime2 = pRecord2.m_flSimTime;
      ...
      if (tMoveStorage.m_MoveData.m_flMaxSpeed)
        flYaw *= std::clamp(pRecord1.m_vVelocity.Length2D() /
                                tMoveStorage.m_MoveData.m_flMaxSpeed,
                            0.f, 1.f);
    ...
    if (flCurrentChance < Vars::Aimbot::Projectile::HitChance.Value / 100) {
      SDK::Output("MovementSimulation",
                  std::format("Hitchance ({}% < {}%)", flCurrentChance * 100,
                              Vars::Aimbot::Projectile::HitChance.Value)
                      .c_str(),
                  {80, 200, 120}, Vars::Debug::Logging.Value);
...
bool CMovementSimulation::SetupMoveData(MoveStorage &tMoveStorage) {
  ...
  tMoveStorage.m_MoveData.m_nPlayerHandle =
      reinterpret_cast<IHandleEntity *>(tMoveStorage.m_pPlayer)
          ->GetRefEHandle();

  tMoveStorage.m_MoveData.m_vecAbsOrigin =
      tMoveStorage.m_pPlayer->m_vecOrigin();
  tMoveStorage.m_MoveData.m_vecVelocity =
      tMoveStorage.m_pPlayer->m_vecVelocity();
  tMoveStorage.m_MoveData.m_flMaxSpeed = SDK::MaxSpeed(tMoveStorage.m_pPlayer);
  tMoveStorage.m_MoveData.m_flClientMaxSpeed =
      tMoveStorage.m_MoveData.m_flMaxSpeed;
  ...
  if (!tMoveStorage.m_MoveData.m_vecVelocity.To2D().IsZero()) {
    ...
    if (iIndex == I::EngineClient->GetLocalPlayer() && G::CurrentUserCmd)
      ...
      if (!tMoveStorage.m_pPlayer->InCond(TF_COND_SHIELD_CHARGE))
        tMoveStorage.m_MoveData.m_vecViewAngles = {
            0.f, Math::VectorAngles(tMoveStorage.m_MoveData.m_vecVelocity).y,
            0.f};
      else
        tMoveStorage.m_MoveData.m_vecViewAngles =
            H::Entities.GetEyeAngles(iIndex);
    }
  ...
  tMoveStorage.m_MoveData.m_vecAngles = tMoveStorage.m_MoveData.m_vecOldAngles =
      tMoveStorage.m_MoveData.m_vecViewAngles;
  if (auto pConstraintEntity =
          tMoveStorage.m_pPlayer->m_hConstraintEntity().Get())
    tMoveStorage.m_MoveData.m_vecConstraintCenter =
        pConstraintEntity->GetAbsOrigin();
  else
    tMoveStorage.m_MoveData.m_vecConstraintCenter =
        tMoveStorage.m_pPlayer->m_vecConstraintCenter();
  tMoveStorage.m_MoveData.m_flConstraintRadius =
      tMoveStorage.m_pPlayer->m_flConstraintRadius();
  tMoveStorage.m_MoveData.m_flConstraintWidth =
      tMoveStorage.m_pPlayer->m_flConstraintWidth();
  tMoveStorage.m_MoveData.m_flConstraintSpeedFactor =
      tMoveStorage.m_pPlayer->m_flConstraintSpeedFactor();
  ...
  tMoveStorage.m_flPredictedSimTime =
      tMoveStorage.m_flSimTime + tMoveStorage.m_flPredictedDelta;
  tMoveStorage.m_vPredictedOrigin = tMoveStorage.m_MoveData.m_vecAbsOrigin;
  tMoveStorage.m_bDirectMove = tMoveStorage.m_pPlayer->IsOnGround() ||
                               tMoveStorage.m_pPlayer->IsSwimming();
...
static inline float GetFrictionScale(float flVelocityXY, float flTurn,
                                     float flVelocityZ, float flMin = 50.f,
                                     float flMax = 150.f) {
...
static inline void VisualizeRecords(MoveData &tRecord1, MoveData &tRecord2,
                                    Color_t tColor,
                                    float flStraightFuzzyValue) {
  ...
  const bool bStraight =
      fabsf(flYaw) * tRecord1.m_vVelocity.Length2D() * iTicks <
      flStraightFuzzyValue; // dumb way to get straight bool

  G::LineStorage.emplace_back(
      std::pair<Vec3, Vec3>(tRecord1.m_vOrigin, tRecord2.m_vOrigin),
      I::GlobalVars->curtime + 5.f, tColor);
  G::LineStorage.emplace_back(
      std::pair<Vec3, Vec3>(tRecord1.m_vOrigin,
                            tRecord1.m_vOrigin + Vec3(0, 0, 5)),
      I::GlobalVars->curtime + 5.f, tColor);
  if (!bStraight && flYaw) {
    Vec3 vVelocity = tRecord1.m_vVelocity.Normalized2D() * 5;
    vVelocity =
        Math::RotatePoint(vVelocity, {}, {0, flYaw > 0 ? 90.f : -90.f, 0});
    if (Vars::Aimbot::Projectile::MovesimFrictionFlags.Value &
            Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::
                CalculateIncrease &&
        tRecord1.m_iMode == MoveEnum::Air)
      vVelocity /= GetFrictionScale(
          tRecord1.m_vVelocity.Length2D(), flYaw,
          tRecord1.m_vVelocity.z + GetGravity() * TICK_INTERVAL, 0.f, 56.f);
    G::LineStorage.emplace_back(
        std::pair<Vec3, Vec3>(tRecord1.m_vOrigin,
                              tRecord1.m_vOrigin + vVelocity),
        I::GlobalVars->curtime + 5.f, tColor);
  }
}
#endif

static inline bool GetYawDifference(MoveData &tRecord1, MoveData &tRecord2,
                                    bool bStart, float *pYaw,
                                    float flStraightFuzzyValue,
                                    int iMaxChanges = 0, int iMaxChangeTime = 0,
  ...
  *pYaw = Math::NormalizeAngle(flYaw1 - flYaw2);
  ...
  if (Vars::Aimbot::Projectile::MovesimFrictionFlags.Value &
          Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::
              CalculateIncrease &&
      tRecord1.m_iMode == 1)
    *pYaw /= GetFrictionScale(
        tRecord1.m_vVelocity.Length2D(), *pYaw,
        tRecord1.m_vVelocity.z + GetGravity() * TICK_INTERVAL, 0.f, 56.f);
  if (fabsf(*pYaw) > 45.f)
    return false;
  ...
  const bool bStraight =
      fabsf(*pYaw) * tRecord1.m_vVelocity.Length2D() * iTicks <
      flStraightFuzzyValue; // dumb way to get straight bool
  ...
  if (bStart) {
    ...
    if ((bChanged || bStraight) && ++iChanges > iMaxChanges)
      return false;
    return iChanges && iStart - TIME_TO_TICKS(flTime2) > iMaxChangeTime ? false
                                                                        : true;
  }
}

void CMovementSimulation::GetAverageYaw(MoveStorage &tMoveStorage,
                                        int iSamples) {
  ...
  bool bGround = tMoveStorage.m_bDirectMove;
  int iMinimumStrafes = 4;
  float flMaxSpeed = SDK::MaxSpeed(tMoveStorage.m_pPlayer, false, true);
  float flLowMinimumDistance =
      bGround ? Vars::Aimbot::Projectile::GroundLowMinimumDistance.Value
              : Vars::Aimbot::Projectile::AirLowMinimumDistance.Value;
  float flLowMinimumSamples =
      bGround ? Vars::Aimbot::Projectile::GroundLowMinimumSamples.Value
              : Vars::Aimbot::Projectile::AirLowMinimumSamples.Value;
  float flHighMinimumDistance =
      bGround ? Vars::Aimbot::Projectile::GroundHighMinimumDistance.Value
              : Vars::Aimbot::Projectile::AirHighMinimumDistance.Value;
  float flHighMinimumSamples =
      bGround ? Vars::Aimbot::Projectile::GroundHighMinimumSamples.Value
  ...
  for (; i < iSamples; i++) {
    ...
    float flStraightFuzzyValue =
        bGround ? Vars::Aimbot::Projectile::GroundStraightFuzzyValue.Value
                : Vars::Aimbot::Projectile::AirStraightFuzzyValue.Value;
    int iMaxChanges = bGround ? Vars::Aimbot::Projectile::GroundMaxChanges.Value
                              : Vars::Aimbot::Projectile::AirMaxChanges.Value;
    int iMaxChangeTime =
        bGround ? Vars::Aimbot::Projectile::GroundMaxChangeTime.Value
    ...
    bool bResult = GetYawDifference(tRecord1, tRecord2, !iTicks, &flYaw,
                                    flStraightFuzzyValue, iMaxChanges,
                                    iMaxChangeTime, flMaxSpeed);
    SDK::Output(
        "GetYawDifference",
        std::format("{} ({}): {}, {}", i, iTicks, flYaw, bResult).c_str(),
        {50, 127, 75}, Vars::Debug::Logging.Value);
    if (!bResult)
      break;

    flAverageYaw += flYaw;
    iTicks +=
        std::max(TIME_TO_TICKS(tRecord1.m_flSimTime - tRecord2.m_flSimTime), 1);
  }
#ifdef VISUALIZE_RECORDS
  size_t i2 = i;
  for (; i2 < iSamples; i2++) {
  ...
  for (; i2 < vRecords.size(); i2++)
  {
          ...
          float flStraightFuzzyValue = bGround ?
  Vars::Aimbot::Projectile::GroundStraightFuzzyValue.Value :
  Vars::Aimbot::Projectile::AirStraightFuzzyValue.Value;
          VisualizeRecords(tRecord1, tRecord2, { 0, 0, 0, 100 },
  flStraightFuzzyValue);
  }
  */
#endif
  ...
  if (pPlayer->entindex() != I::EngineClient->GetLocalPlayer()) {
    ...
    if (auto pLocal = H::Entities.GetLocal())
      flDistance =
          pLocal->m_vecOrigin().DistTo(tMoveStorage.m_pPlayer->m_vecOrigin());
    iMinimum =
        flDistance < flLowMinimumDistance
            ? flLowMinimumSamples
            : Math::RemapVal(flDistance, flLowMinimumDistance,
                             flHighMinimumDistance, flLowMinimumSamples + 1,
                             flHighMinimumSamples);
  }
  ...
  SDK::Output(
      "MovementSimulation",
      std::format("flAverageYaw calculated to {} from {} ({}) {}", flAverageYaw,
                  iTicks, iMinimum,
                  pPlayer->entindex() == I::EngineClient->GetLocalPlayer()
                      ? "(local)"
                      : "")
          .c_str(),
      {100, 255, 150}, Vars::Debug::Logging.Value);
}

bool CMovementSimulation::StrafePrediction(MoveStorage &tMoveStorage,
                                           int iSamples) {
  if (tMoveStorage.m_bDirectMove
          ? !(Vars::Aimbot::Projectile::StrafePrediction.Value &
              Vars::Aimbot::Projectile::StrafePredictionEnum::Ground)
          : !(Vars::Aimbot::Projectile::StrafePrediction.Value &
              Vars::Aimbot::Projectile::StrafePredictionEnum::Air))
    return false;
...
bool CMovementSimulation::SetDuck(
    MoveStorage &tMoveStorage, bool bDuck) // this only touches origin, bounds
{
  ...
  if (!tMoveStorage.m_pPlayer->IsOnGround()) {
    Vec3 vHullMins =
        (pViewVectors ? pViewVectors->m_vHullMin : Vec3(-24, -24, 0)) * flScale;
    Vec3 vHullMaxs =
        (pViewVectors ? pViewVectors->m_vHullMax : Vec3(24, 24, 82)) * flScale;
    Vec3 vDuckHullMins =
        (pViewVectors ? pViewVectors->m_vDuckHullMin : Vec3(-24, -24, 0)) *
        flScale;
    Vec3 vDuckHullMaxs =
        (pViewVectors ? pViewVectors->m_vDuckHullMax : Vec3(24, 24, 62)) *
        flScale;

    if (bDuck)
      tMoveStorage.m_MoveData.m_vecAbsOrigin +=
          (vHullMaxs - vHullMins) - (vDuckHullMaxs - vDuckHullMins);
    else {
      Vec3 vOrigin =
          tMoveStorage.m_MoveData.m_vecAbsOrigin -
          ((vHullMaxs - vHullMins) - (vDuckHullMaxs - vDuckHullMins));
      ...
      CGameTrace trace = {};
      CTraceFilterWorldAndPropsOnly filter = {};
      SDK::TraceHull(vOrigin, vOrigin, vHullMins, vHullMaxs,
...
void CMovementSimulation::SetBounds(CTFPlayer *pPlayer) {
  ...
  if (auto pGameRules = I::TFGameRules()) {
    if (auto pViewVectors = pGameRules->GetViewVectors()) {
      ...
      pViewVectors->m_vDuckHullMin =
          Vec3(-24, -24, 0) + PLAYER_ORIGIN_COMPRESSION;
      pViewVectors->m_vDuckHullMax =
          Vec3(24, 24, 62) - PLAYER_ORIGIN_COMPRESSION;
    }
  }
}
...
void CMovementSimulation::RunTick(MoveStorage &tMoveStorage, bool bPath,
                                  std::function<void(CMoveData &)> *pCallback) {
  if (tMoveStorage.m_bFailed || !tMoveStorage.m_pPlayer ||
      !tMoveStorage.m_pPlayer->IsPlayer())
    return;
  ...
  I::GlobalVars->frametime =
      I::Prediction->m_bEnginePaused ? 0.f : TICK_INTERVAL;
  ...
  if (tMoveStorage.m_flAverageYaw) {
    float flMult = 1.f;
    if (!tMoveStorage.m_bDirectMove &&
        !tMoveStorage.m_pPlayer->InCond(TF_COND_SHIELD_CHARGE)) {
      flCorrection = 90.f * sign(tMoveStorage.m_flAverageYaw);
      if (Vars::Aimbot::Projectile::MovesimFrictionFlags.Value &
          Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::RunReduce)
        flMult =
            GetFrictionScale(tMoveStorage.m_MoveData.m_vecVelocity.Length2D(),
                             tMoveStorage.m_flAverageYaw,
                             tMoveStorage.m_MoveData.m_vecVelocity.z +
                                 GetGravity() * TICK_INTERVAL);
    }
    tMoveStorage.m_MoveData.m_vecViewAngles.y +=
        tMoveStorage.m_flAverageYaw * flMult + flCorrection;
  } else if (!tMoveStorage.m_bDirectMove)
    tMoveStorage.m_MoveData.m_flForwardMove =
        tMoveStorage.m_MoveData.m_flSideMove = 0.f;

  float flOldSpeed = tMoveStorage.m_MoveData.m_flClientMaxSpeed;
  if (tMoveStorage.m_pPlayer->m_bDucked() &&
      tMoveStorage.m_pPlayer->IsOnGround() &&
      !tMoveStorage.m_pPlayer->IsSwimming())
    tMoveStorage.m_MoveData.m_flClientMaxSpeed /= 3;

  if (tMoveStorage.m_bBunnyHop && tMoveStorage.m_pPlayer->IsOnGround() &&
      !tMoveStorage.m_pPlayer->m_bDucked()) {
    tMoveStorage.m_MoveData.m_nOldButtons = 0;
    tMoveStorage.m_MoveData.m_nButtons |= IN_JUMP;
  }

  I::GameMovement->ProcessMovement(tMoveStorage.m_pPlayer,
                                   &tMoveStorage.m_MoveData);
  if (pCallback)
  ...
  tMoveStorage.m_bPredictNetworked =
      tMoveStorage.m_flSimTime >= tMoveStorage.m_flPredictedSimTime;
  ...
  if (tMoveStorage.m_flAverageYaw)
    tMoveStorage.m_MoveData.m_vecViewAngles.y -= flCorrection;
           ...
           !tMoveStorage.m_MoveData.m_flForwardMove &&
           !tMoveStorage.m_MoveData.m_flSideMove &&
           tMoveStorage.m_MoveData.m_vecVelocity.Length2D() >
               tMoveStorage.m_MoveData.m_flMaxSpeed * 0.015f) {
    Vec3 vDirection =
        tMoveStorage.m_MoveData.m_vecVelocity.Normalized2D() * 450.f;
    s_tDummyCmd.forwardmove = vDirection.x,
    s_tDummyCmd.sidemove = -vDirection.y;
...
void CMovementSimulation::RunTick(MoveStorage &tMoveStorage, bool bPath,
                                  std::function<void(CMoveData &)> fCallback) {
