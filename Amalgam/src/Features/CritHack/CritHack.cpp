...
int CCritHack::GetCritCommand(CTFWeaponBase *pWeapon, int iCommandNumber,
                              bool bCrit, bool bSafe) {
...
bool CCritHack::IsCritCommand(int iCommandNumber, CTFWeaponBase *pWeapon,
                              bool bCrit, bool bSafe) {
  int iSeed = CommandToSeed(iCommandNumber);
  return IsCritSeed(iSeed, pWeapon, bCrit, bSafe);
}

bool CCritHack::IsCritSeed(int iSeed, CTFWeaponBase *pWeapon, bool bCrit,
                           bool bSafe) {
...
int CCritHack::CommandToSeed(int iCommandNumber) {
  int iSeed =
      MD5_PseudoRandom(iCommandNumber) & std::numeric_limits<int>::max();
  int iMask = m_bMelee
...
void CCritHack::UpdateWeaponInfo(CTFPlayer *pLocal, CTFWeaponBase *pWeapon) {
  ...
  if (m_bMelee)
    ...
    float flNonCritDuration = (TF_DAMAGE_CRIT_DURATION_RAPID / m_flCritChance) -
  ...
  const int iCritSeedRequests = iStaticCritSeedRequests =
      pWeapon->m_nCritSeedRequests();

  if (pWeapon == pOldWeapon && flBucket == flLastBucket &&
      iCritChecks == iLastCritChecks &&
      iCritSeedRequests == iLastCritSeedRequests)
    return;

  static auto tf_weapon_criticals_bucket_cap =
      H::ConVars.FindVar("tf_weapon_criticals_bucket_cap");
  ...
  if (!m_bMelee && nProjectilesPerShot > 0)
    nProjectilesPerShot = SDK::AttribHookValue(
        nProjectilesPerShot, "mult_bullets_per_shot", pWeapon);
  else
  ...
  float flMult =
      m_bMelee
          ? 0.5f
          : Math::RemapVal(float(iCritSeedRequests + 1) / (iCritChecks + 1),
                           0.1f, 1.f, 1.f, 3.f);
  float flCost = flDamage * TF_DAMAGE_CRIT_MULTIPLIER;

  int iPotentialCrits =
      (std::max(flBucketCap, flBucket) - flBaseDamage) /
      (TF_DAMAGE_CRIT_MULTIPLIER * flDamage / (m_bMelee ? 2 : 1) -
       flBaseDamage);
  int iAvailableCrits = 0;
  {
    ...
    for (int i = 0; i < BUCKET_ATTEMPTS; i++) {
      ...
      float flTestMult = m_bMelee
                             ? 0.5f
                             : Math::RemapVal(float(iTestCrits) / iTestShots,
                                              0.1f, 1.f, 1.f, 3.f);
  ...
  if (iAvailableCrits != iPotentialCrits) {
    ...
    float flLastRapidFireCritCheckTime =
        pWeapon->m_flLastRapidFireCritCheckTime();
    for (int i = 0; i < BUCKET_ATTEMPTS; i++) {
      int iCrits = 0;
      {
        ...
        for (int j = 0; j < BUCKET_ATTEMPTS; j++) {
          ...
          float flTestMult =
              m_bMelee ? 0.5f
                       : Math::RemapVal(float(iTestCrits2) / iTestShots2, 0.1f,
                                        1.f, 1.f, 3.f);
      ...
      if (!bRapidFire)
        ...
        if (flTickBase >= flLastRapidFireCritCheckTime + 1.f ||
            !i && flTestBucket == flBucketCap) {
          iTestShots++;
...
void CCritHack::UpdateInfo(CTFPlayer *pLocal, CTFWeaponBase *pWeapon) {
  ...
  if (!m_bMelee) {
    ...
    if (m_iRangedDamage && m_iCritDamage) {
      const float flObservedCritChance =
          flNormalizedDamage /
          (flNormalizedDamage + m_iRangedDamage - m_iCritDamage);
      m_bCritBanned = flObservedCritChance > flCritChance;
    }

    if (m_bCritBanned)
      m_flDamageTilFlip = flNormalizedDamage / flCritChance +
                          flNormalizedDamage * 2 - m_iRangedDamage;
    else {
      if (fabsf(flCritChance - 1.f) < 0.001f)
        m_flDamageTilFlip = 0;
      else
        m_flDamageTilFlip = TF_DAMAGE_CRIT_MULTIPLIER *
                            (flNormalizedDamage -
                             flCritChance * (flNormalizedDamage +
                                             m_iRangedDamage - m_iCritDamage)) /
                            (flCritChance - 1);
    }
  }
  ...
  if (auto pResource = H::Entities.GetResource()) {
    ...
    {	// attempt to resync damages
            ...
            m_iCritDamage = (TF_DAMAGE_CRIT_MULTIPLIER * flObservedCritChance *
    m_iResourceDamage) / (1 + 2 * flObservedCritChance); SDK::Output("Info",
    std::format("{}, {}", m_iRangedDamage, m_iCritDamage).c_str());
    }
    */
...
bool CCritHack::WeaponCanCrit(CTFWeaponBase *pWeapon, bool bWeaponOnly) {
  if (!bWeaponOnly && !pWeapon->AreRandomCritsEnabled() ||
      SDK::AttribHookValue(1.f, "mult_crit_chance", pWeapon) <= 0.f)
    return false;
...
int CCritHack::GetCritRequest(CUserCmd *pCmd, CTFWeaponBase *pWeapon) {
  ...
  if (Vars::CritHack::AlwaysMeleeCrit.Value && m_bMelee &&
      (Vars::Aimbot::General::AutoShoot.Value
           ? pCmd->buttons & IN_ATTACK && !(G::OriginalCmd.buttons & IN_ATTACK)
           : Vars::Aimbot::General::AimType.Value) &&
      G::AimTarget.m_iEntIndex) {
    auto pEntity =
        I::ClientEntityList->GetClientEntity(G::AimTarget.m_iEntIndex)
            ->As<CBaseEntity>();
  ...
  bool bDesync =
      CommandToSeed(pCmd->command_number) == pWeapon->m_iCurrentSeed();

  return bCanCrit && bPressed ? CritRequestEnum::Crit
         : bSkip || bDesync   ? CritRequestEnum::Skip
                              : CritRequestEnum::Any;
}
...
void CCritHack::Run(CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd) {
  ...
  bool bAttacking =
      G::Attacking /*== 1*/ || F::Ticks.m_bDoubletap || F::Ticks.m_bSpeedhack;
  if (m_bMelee) {
    ...
    case TF_WEAPON_ROCKETLAUNCHER_DIRECTHIT:
      if (pWeapon->IsInReload() && G::CanPrimaryAttack &&
          SDK::AttribHookValue(0, "can_overload", pWeapon)) {
        int iClip1 = pWeapon->m_iClip1();
        if (pWeapon->m_bRemoveable() && iClip1 > 0)
          bAttacking = true;
        else if (iClip1 >= pWeapon->GetMaxClip1() ||
                 iClip1 > 0 &&
                     pLocal->GetAmmoCount(pWeapon->m_iPrimaryAmmoType()) == 0)
          bAttacking = true;
      }
    }
  }
  if (!bAttacking || pWeapon->IsRapidFire() &&
                         I::GlobalVars->curtime <
                             pWeapon->m_flLastRapidFireCritCheckTime() + 1.f)
    return;
  ...
  if (!Vars::Misc::Game::AntiCheatCompatibility.Value) {
    if (int iCommand = GetCritCommand(pWeapon, pCmd->command_number,
                                      iRequest == CritRequestEnum::Crit)) {
      pCmd->command_number = iCommand;
      pCmd->random_seed =
          MD5_PseudoRandom(iCommand) & std::numeric_limits<int>::max();
    }
  } else if (Vars::Misc::Game::AntiCheatCritHack.Value) {
    if (!IsCritCommand(pCmd->command_number, pWeapon,
                       iRequest == CritRequestEnum::Crit, false)) {
...
int CCritHack::PredictCmdNum(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                             CUserCmd *pCmd) {
  auto getCmdNum = [&](int iCommandNumber) {
    ...
    if (int iCommand = GetCritCommand(pWeapon, iCommandNumber,
                                      iRequest == CritRequestEnum::Crit))
      return iCommand;
    return iCommandNumber;
  };
...
void CCritHack::Event(IGameEvent *pEvent, uint32_t uHash, CTFPlayer *pLocal) {
  ...
  case FNV1A::Hash32Const("player_hurt"): {
    ...
    if (m_mHealthHistory.contains(iVictim)) {
      ...
      auto pVictim =
          I::ClientEntityList->GetClientEntity(iVictim)->As<CTFPlayer>();

      if (!iHealth) {
        iDamage = Math::Clamp(iDamage, 0, tHistory.m_iNewHealth);
        tHistory.m_iSpawnCounter = -1;
      } else if (pVictim &&
                 (pVictim->m_bFeignDeathReady() ||
                  pVictim->InCond(
                      TF_COND_FEIGN_DEATH))) // damage number is spoofed upon
                                             // sending, correct it
      {
        int iOldHealth = (tHistory.m_mHistory.contains(iHealth)
                              ? tHistory.m_mHistory[iHealth].m_iOldHealth
                              : tHistory.m_iNewHealth) %
                         32768;
        if (iHealth > iOldHealth) {
          for (auto &[_, tOldHealth] : tHistory.m_mHistory) {
            ...
            if (iOldHealth2 > iHealth)
              iOldHealth = iHealth > iOldHealth
                               ? iOldHealth2
                               : std::min(iOldHealth, iOldHealth2);
          }
        }
        iDamage = Math::Clamp(iOldHealth - iHealth, 0, iDamage);
      }
    }
    if (iHealth)
    ...
    if (auto pGameRules = I::TFGameRules()) {
      ...
      const int iInsanePlayerDamage =
          pGameRules->m_bPlayingMannVsMachine() ? 5000 : 1500;
...
void CCritHack::StoreHealthHistory(int iIndex, int iHealth,
                                   CTFPlayer *pPlayer) {
  bool bContains = m_mHealthHistory.contains(iIndex);
  auto &tHistory = m_mHealthHistory[iIndex];

  if (bContains && pPlayer) { // deal with instant respawn damage desync better
  ...
  tHistory.m_mHistory[iHealth % 32768] = {tHistory.m_iOldHealth,
                                          float(SDK::PlatFloatTime())};
...
MAKE_SIGNATURE(CTFGameStats_FindPlayerStats, "server.dll",
               "4C 8B C1 48 85 D2 75", 0x0);
MAKE_SIGNATURE(UTIL_PlayerByIndex, "server.dll",
               "48 83 EC ? 8B D1 85 C9 7E ? 48 8B 05", 0x0);

static void *s_pCTFGameStats = nullptr;
MAKE_HOOK(CTFGameStats_FindPlayerStats, S::CTFGameStats_FindPlayerStats(),
          void *, void *rcx, CBasePlayer *pPlayer) {
...
void CCritHack::Draw(CTFPlayer *pLocal) {
  if (!(Vars::Menu::Indicators.Value & Vars::Menu::IndicatorsEnum::CritHack) ||
      !I::EngineClient->IsInGame())
    return;
  ...
  if (!pWeapon->AreRandomCritsEnabled()) {
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value,
        Vars::Menu::Theme::Background.Value, align, "Random crits disabled");
    return;
  }
  ...
  if (Vars::Misc::Game::AntiCheatCompatibility.Value)
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value,
        Vars::Menu::Theme::Background.Value, align, "Anticheat compatibility");

  if (pLocal->IsCritBoosted())
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Colors::IndicatorTextMisc.Value,
        Vars::Menu::Theme::Background.Value, align, "Crit Boosted");
  else if (pWeapon->m_flCritTime() > flTickBase) {
    float flTime = pWeapon->m_flCritTime() - flTickBase;
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Colors::IndicatorTextMisc.Value,
        Vars::Menu::Theme::Background.Value, align,
        std::format("Streaming crits {:.1f}s", flTime).c_str());
  } else if (!m_bCritBanned) {
    if (m_iPotentialCrits > 0) {
      if (m_iAvailableCrits > 0) {
        if (!pWeapon->IsRapidFire() ||
            flTickBase >= pWeapon->m_flLastRapidFireCritCheckTime() + 1.f)
          H::Draw.StringOutlined(
              fFont, x, y += nTall, Vars::Colors::IndicatorTextGood.Value,
              Vars::Menu::Theme::Background.Value, align, "Crit Ready");
        else {
          float flTime =
              pWeapon->m_flLastRapidFireCritCheckTime() + 1.f - flTickBase;
          H::Draw.StringOutlined(fFont, x, y += nTall,
        ...
        H::Draw.StringOutlined(fFont, x, y += nTall,
                               ...
                               Vars::Menu::Theme::Background.Value, align,
                               std::format("Crit in {}{} shot{}", iShots,
                                           iShots == BUCKET_ATTEMPTS ? "+" : "",
                                           iShots == 1 ? "" : "s")
                                   .c_str());
      }
    }
  ...
  if (m_iPotentialCrits > 0) {
    ...
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Menu::Theme::Active.Value,
        Vars::Menu::Theme::Background.Value, align,
        std::format("{}{} / {} crits", iCrits,
                    iCrits == BUCKET_ATTEMPTS ? "+" : "", m_iPotentialCrits)
            .c_str());
    ...
    if (m_iNextCrit && iCrits) {
      ...
      H::Draw.StringOutlined(fFont, x, y += nTall,
                             Vars::Menu::Theme::Active.Value,
                             Vars::Menu::Theme::Background.Value, align,
                             std::format("Next in {}{} shot{}", iShots,
                                         iShots == BUCKET_ATTEMPTS ? "+" : "",
                                         iShots == 1 ? "" : "s")
                                 .c_str());
    }
  ...
  if (m_iDesyncDamage) {
    auto tColor =
        m_iDesyncDamage < 0
            ? Vars::Menu::Theme::Active.Value.Lerp(
                  Vars::Colors::IndicatorTextMid.Value,
                  std::min(static_cast<float>(abs(m_iDesyncDamage)) / 100, 1.f))
            : Vars::Colors::IndicatorTextBad.Value;
    H::Draw.StringOutlined(fFont, x, y += nTall, tColor,
                           Vars::Menu::Theme::Background.Value, align,
                           std::format("{}{} desync",
                                       m_iDesyncDamage > 0 ? "+" : "",
                                       m_iDesyncDamage)
  ...
  if (Vars::Debug::Info.Value) {
    H::Draw.StringOutlined(fFont, x, y += nTall * 2,
                           Vars::Menu::Theme::Active.Value,
                           Vars::Menu::Theme::Background.Value, align,
                           std::format("RangedDamage: {}, CritDamage: {}",
                                       m_iRangedDamage, m_iCritDamage)
                               .c_str());
    ...
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Menu::Theme::Active.Value,
        Vars::Menu::Theme::Background.Value, align,
        std::format("AllDamage: {} ({})", m_iRangedDamage + m_iMeleeDamage,
                    m_iMeleeDamage)
            .c_str());

    if (s_pCTFGameStats) {
      if (auto pPlayer2 = S::UTIL_PlayerByIndex.Call<void *>(
              I::EngineClient->GetLocalPlayer())) {
        if (auto pPlayerStats =
                S::CTFGameStats_FindPlayerStats.Call<PlayerStats_t *>(
                    s_pCTFGameStats, pPlayer2)) {
          int &iRangedDamage =
              pPlayerStats->statsCurrentRound.m_iStat[TFSTAT_DAMAGE_RANGED];
          int &iCritDamage = pPlayerStats->statsCurrentRound
                                 .m_iStat[TFSTAT_DAMAGE_RANGED_CRIT_RANDOM];
          ...
          H::Draw.StringOutlined(fFont, x, y += nTall,
                                 ...
                                 Vars::Menu::Theme::Background.Value, align,
                                 std::format("RangedDamage: {}, CritDamage: {}",
                                             iRangedDamage, iCritDamage)
                                     .c_str());
          H::Draw.StringOutlined(fFont, x, y += nTall,
                                 Vars::Menu::Theme::Active.Value,
                                 Vars::Menu::Theme::Background.Value, align,
                                 std::format("AllDamage: {} ({})", iDamage,
                                             iDamage - iRangedDamage)
                                     .c_str());
        }
      }
    }
    ...
    H::Draw.StringOutlined(fFont, x, y += nTall,
                           Vars::Menu::Theme::Active.Value,
                           Vars::Menu::Theme::Background.Value, align,
                           std::format("ResourceDamage: {} ({})",
                                       m_iResourceDamage, m_iMeleeDamage)
                               .c_str());
#endif

    H::Draw.StringOutlined(
        fFont, x, y += nTall * 2, Vars::Menu::Theme::Active.Value,
        Vars::Menu::Theme::Background.Value, align,
        std::format("Bucket: {}, Shots: {}, Crits: {}",
                    pWeapon->m_flCritTokenBucket(), pWeapon->m_nCritChecks(),
                    pWeapon->m_nCritSeedRequests())
            .c_str());
    H::Draw.StringOutlined(
        fFont, x, y += nTall, Vars::Menu::Theme::Active.Value,
    ...
    H::Draw.StringOutlined(fFont, x, y += nTall,
                           Vars::Menu::Theme::Active.Value,
                           Vars::Menu::Theme::Background.Value, align,
                           std::format("CritChance: {:.2f} ({:.2f})",
                                       m_flCritChance, m_flCritChance + 0.1f)
                               .c_str());
  }
}